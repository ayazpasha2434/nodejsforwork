...
function add_to_mongo(docId, city, thumbnail, params, callback) {
    ...
    try {
        ...
        if(db != undefined) {
            ...
            db.collection('babysitters_new').find({_id: docId}).toArray(function(err, sitters) {
                ...
                if(!err) {
                    ...
                    db.collection('babysitters_new').update(query, operator, options, function(err, upserted) {
                        if (err) {
                            throw err;
                            callback(err);
                        } else {
            ...
            MongoClient.connect('mongodb://127.0.0.1:27017/babysitter', function(err, dbc) {
                ...
                dbc.collection('babysitters_new').find({_id: docId}).toArray(function(err, sitters) {
                    ...
                    if(!err) {
                        ...
                        dbc.collection('babysitters_new').update(query, operator, options, function(err, upserted) {
                            if (err) {
                                throw err;
                                callback(err);
                            } else {
        ...
        console.error("babysitters_new/insertOrUpdate Error: "+e);
        callback(e);
    }
}
