...
function add_to_mongo(docId, city, thumbnail, params, callback) {
    ...
    try {
        ...
        if(db != undefined) {
            ...
            db.collection('elderly_care').find({_id: docId}).toArray(function(err, sitters) {
                ...
                if(!err) {
                    ...
                    db.collection('elderly_care').update(query, operator, options, function(err, upserted) {
                        if (err) {
                            throw err;
                            callback(err);
                        } else {
            ...
            MongoClient.connect('mongodb://127.0.0.1:27017/babysitter', function(err, dbc) {
                ...
                dbc.collection('elderly_care').find({_id: docId}).toArray(function(err, sitters) {
                    ...
                    if(!err) {
                        ...
                        dbc.collection('elderly_care').update(query, operator, options, function(err, upserted) {
                            if (err) {
                                throw err;
                                callback(err);
                            } else {
        ...
        console.error("elderly_care/insertOrUpdate Error: "+e);
        callback(e);
    }
}
